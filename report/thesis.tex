...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.45\textwidth]{../figures/buffer_tree_fuse}
...
Solving the problem of three-sided range queries is very closely related to that of the 1D-dictionary problem. We can reduce the three-sided range reporting problem to the 1D-dictionary by restricting elements to be of the form $(x,x)$ and test membership by a query of the form $\left[x,x\right] \times \left[-\infty, \infty\right]$.
%Any solution that solves the three-sided range query problem can solve the 1D-dictionary problem by testing membership of an element $(x,y)$ by reporting all points in the range $\left[x,x\right] \times \left[y, \infty\right]$. 
By reduction we must have that the lower bounds of the 1D-dictionary problem also applies to that of three-sided range queries.

The 1D-dictionary problem has been a popular topic in the case of internal memory. It has been proved by a simply adversary argument that a query can be forced to cost $\log_2 N$ comparisons no matter the cost of updates, and more generally it has been proved that if an insertion perform at most $\mathcal{O}(k)$ comparisons then queries can be forced to cost at least $\max\left\lbrace \log_2 N, N/2^{\Theta(k)}\right\rbrace$ comparisons~\cite{Borodin81efficientsearching}.
...
\begin{figure}[h]
	...
	\caption{The grey area is our query and we should report the points within. This is done by finding the fat line segments which is the segments just below the sweep line at $y$. The segments can be found using the catalog and the blocks can be scanned and relevant points can be reported in $\mathcal{O}(1+\nicefrac{K}{B})$.}
...
\begin{figure}[t]
	...
	\caption{Bubble down. First a degenerate query on the form $\left[ x\text{-range of }w \right] \times \left[ - \infty, \infty \right]$ is done for the found child $w$ resulting in a set of points with size $K$ (a) If $K \leq B/2$ then we simply insert the point (here marked as a circle) (b) If $K \geq B/2$ and the point is \textit{larger} the \textit{smallest} $y$-value then the point is inserted and the smallest $y$-value is recursively sent down (c) If $K \geq B/2$ and the point is \textit{smaller} the \textit{smallest} $y$-value then the point itself is sent recursively down.}
...
We identify which nodes to visit by searching on a path from the root to leaf along paths corresponding to $x_1$ and $x_2$ and visit nodes in between the two paths. As the tree is a search tree on the $x$-coordinate we know that nodes in between the search paths for $x_1$ and $x_2$ will be in the query range of $Q$.
...
We update the structure with insertions and deletions by adding points to either the insertion or deletion buffer respectively of the root, while maintaining the above invariants.
...
The $\mathcal{O}(\bar{N})$ reinsertions into the new initially empty tree can be done in amortized $\mathcal{O}(\frac{\bar{N}}{\epsilon B^{1-\epsilon}}\log_B\bar{N})$ I/O's which is paid by the $\bar{N}/2$ updates during the epoch.
...
The structure can be initialized with a set of $N$ points using just $\mathcal{O}(\text{Sort}(N))$ I/O's. If the points are sorted on $x$ we only need $\mathcal{O}(\text{Scan}(N))$. For the remainder of this section we assume that the points are initially sorted with regards to $x$.
...
For buffer size 32 Mb we see more fluctuations and we can hardly argue the plot follows a straight line. We believe this is caused by the limited amount of internal memory forcing us to utilize more than available, which again causes the operating system to swap out internal memory to external memory. 

We verified this by looking at the measured number of page faults generated, and we could see that, just as suspected, the data structure started to cause a low amount of page faults that can explain the fluctuations. See Appendix~\ref{appendix:
\todo{Kasper: say something about why we use so much memory for a buffer size of 32mb when it does not seem like much}

In order to verify this we repeated the experiment for buffer sizes 8 Mb and 16 Mb, this time measuring the number of major page faults. The result is depicted in Figure \todo{CREATE THIS EXPERIMENT AND EXPLAIN}.

\todo{Kasper: Conclude this section with what we are going to use going forward}
...
\todo{Kasper: Conclude with what we are going to use going forward}
...
\todo{Summarize best choices and what we will use forward}
