...
Figure~\ref{fig:actual_insert_time} shows the actual running time per inserted Mb in the 7 tested structures. The figure was cropped at 600Mb to show the relation between the internal memory and the external memory structures. MySQL without an index kept going up to 10Gb with no change in running time per megabyte as suspected. The same goes for the External Memory Buffered Priority Search by Brodal but it stopped at around 3500Mb when it had run for 24 hours.

The internal memory data structures perform very well while contained in memory. Figure~\ref{fig:insert_page_faults} shows the number of page faults generated by the internal memory data structures. The figure shows a correlation between the running time and the number of page faults as the running time increases significantly at the same input size as the number of page faults increases.
