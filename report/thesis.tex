...
\section{Construction}
An interesting feature of the External Memory Buffered Priority Search Tree is the construction in $\mathcal{O}(Scan(N))$ on sorted input. As a small extra experiment we decided to compare this linear construction method on $N$ elements to the insertion of $N$ elements. As we mention in Subsection~\ref{subsec:gerth_construction} we could use this linear construction method together with a $\mathcal{O}(Sort(N))$ algorithm to global rebuild the structure. Our hopes for this experiment is to get an indication of which method is best in terms of global rebuilding.
