...
The theoretical search times on each of the data structures are depicted in Figure~\ref{fig:theory_query_complexity}. Even though the theoretical asymptotic bound suggests the R*-Tree to be linear in the size of the input, we expect it to have an actual average running time closer to $\mathcal{O}(\log_B N)$ guaranteed when no bounding boxes overlap each other. We expect the external memory data structures to perform much better than the internal memory data structures when the operating system starts swapping of main memory.
...
Clearly we see a tendency of faster searches on increasing fanouts.
...
The second experiment on the \textbf{search} part of the algorithms was conducted by fixing $x_1$, $x_2$ and $y_1, \cdots y_n$ and inserting data points such that there is exactly 
1 Mb data points in the range $[x_1, x_2] \times [y_i, y_{i+1}]$. In the range outside  $[x_1, x_2] \times [- \infty, \infty]$ we distribute data points uniformly at random. Now we report points in ranges $[x_1, x_2] \times [y_1, \infty]$, $[x_1, x_2] \times [y_2, \infty]$ $\cdots$ $[x_1, x_2] \times [y_n, \infty]$. The idea is that the number of reported points $K$ only grows with 1 Mb when reporting in the range $[x_1, x_2] \times [y_{i+1}, \infty]$ compared to reporting in the range $[x_1, x_2] \times [y_i, \infty]$.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.8\textwidth]{../figures/query_uniform_fix_x}
	\caption{Distribution of points for the uniform reporting experiment (searching). The points $x_1, x_2$ and $y_1, \cdots y_n$ are fixed. There is 1 Mb data in all ranges $[x_1, x_2] \times [y_i, y_{i+1}]$}.
	\label{fig:experiment_query_uniform_fix_x}
\end{figure}

We first conducted the experiment with 50Mb data distributed \textit{inside} the query windows and an additional 50Mb data distributed \textit{outside} the query windows. The result of this experiment is depicted in Figure~\ref{fig:query_report_100mb}. Clearly we see the Internal Memory Priority Search Tree of McCreight suffers severely from swapping of memory, and so we rule this data structure out after having deleted only 2Mb worth of data. Also, it is obvious that the Boost R-Tree is still able to fit entirely in internal memory which achieves a superior running time.
Based on this observation did we add an additional 300Mb data \textit{outside} the query windows giving a total of 400Mb data. The results of running this experiment on the remaining data structures is depicted in Figure~\ref{fig:query_report_400mb}. It is obvious that the Boost R-Tree can no longer fit into internal memory which results in much worse running times than achieved by the external memory data structures. What is also clear is that MySQL using no index is still superior compared to the data structure of Brodal.
The last experiment conducted was by adding an additional 2100Mb data \textit{outside} the query windows yielding a data structure of a total of 2,500Mb worth of data. The experiment was then run on MySQL with no index and the data structure by Brodal with a fanout of 2 and 4 respectively. The result of this experiment is depicted in Figure~\ref{fig:query_report_2500mb}. It is obvious that the data structure of Brodal now outperforms MySQL which is probably explained by the fact it is able to exclude searching in the parts of the tree with no interest based the $y$-query. Again, we see a clear tendency of increasing performance on query when the fanout goes up. This aligns well with the theoretical analysis.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{../src/experiments/query_experiment_report_50mb_results/Final_100mb/time}
\caption{Reporting as depicted in Figure~\ref{fig:experiment_query_uniform_fix_x} on a data structure of size 100Mb. The Internal Priority Search Tree of McCreight suffers from swapping of memory.}
...
\caption{Reporting as depicted in Figure~\ref{fig:experiment_query_uniform_fix_x} on a data structure of size 400Mb. Here the Boost R-Tree suffers from swapping of memory.}
...
\caption{Reporting as depicted in Figure~\ref{fig:experiment_query_uniform_fix_x} on a data structure of size 2,500Mb. The data structure of Brodal's clearly outperforms MySQL with no index.}
...
\captionsetup{width=0.40\textwidth}
	\centering
		\includegraphics[width=0.4\textwidth]{../figures/query_uniform_fix_x_no_noise}
...
This experiment focuses on the \textbf{report} of the query algorithm. We limited the experiment to only include Brodal's since we had to construct the data structure on a large input size. The experiment reused the idea of distributing data \textit{inside} fixed query windows. The main difference was that we distributed 100Mb uniformly random data \textit{inside} each query window and no data was added \textit{outside} the query windows. The idea is that we report in a $x$-range that spans all points and on increasing query-$y$ values such that the number of reported points, $K$, grows with 100Mb when reporting in the range $[x_1, x_2] \times [y_{i+1}, \infty]$ compared to reporting in the range $[x_1, x_2] \times [y_i, \infty]$. See Figure~\ref{fig:query_report_experimental_setup}.

The result of the experiment is depicted in Figure~\ref{fig:query_report_result}. Clearly we see a linear query time as a function of $K$ which is what we would expect.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{../src/experiments/query_experiment_report_50mb_results/2016-06-09.15_38_51/time}
\caption{Experimentally measured running time when reporting increasing data on Brodal's of input size 1,200Mb.}
