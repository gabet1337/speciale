...
\caption{Distribution of points for the uniform reporting experiment (reporting). The points $x_1, x_2$ and $y_1, \cdots, y_n$ are fixed. There is 100Mb data in all ranges $[x_1, x_2] \times [y_i, y_{i+1}]$.}
	\label{fig:query_report_experimental_setup}
\end{wrapfigure}
...
\section{Construction}
An interesting feature of the External Memory Buffered Priority Search Tree is the construction in $\mathcal{O}(\text{Scan})$ on sorted input. As a small extra experiment we decided to compare this linear construction method on $N$ elements to the insertion of $N$ elements. As we mention in Subsection~\ref{subsec:gerth_construction} we could use this linear construction method together with a $\mathcal{O}(\text{Sort})$ algorithm to global rebuild the structure. Our hopes for this experiment is to get an indication of which method is best in terms of global rebuilding.

The experiment consists of constructing a tree using the linear construction method and inserting for increasing input sizes, $N$, and different fanouts to see whether this has an impact. We know from previous experiments that inserting becomes slower for larger fanouts but it is not clear what happens when we use the linear construction method.
...
Figure~\ref{fig:linear_construction_time_divided_N} depicts how the linear construction method does for different fanouts. The running time is divided by the input size. Since the construction method is linear we are very pleased to see that the graphs are horizontal when divided by $N$. This would indeed corroborate that we can construct the structure in linear time on sorted input. We see that the running time improves for larger fanouts. This is explained in the decreased number of nodes in the tree which means that we see a decreased number of point buffer underflows. Figure~\ref{fig:linear_construction_pbu} supports this claim.

We can conclude from the fact that linear construction is faster than inserting that it is indeed beneficial to construct the structure using the linear construction method when we have sorted input data. In order to conclude that this method should be used for global rebuilding we have to argue about the time it takes to sort the data of the tree such that the method can be used. From an earlier course we have experimentally found that we can indeed sort fast enough for the linear construction method to be viable. The results from this course are depicting in Figure~\ref{fig:linear_construction_sorting}. We conclude that the linear construction method is superior when it comes to global rebuilding.
