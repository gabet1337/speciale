...
It follows from the definition that if $b=\Theta(B)$ then a B-tree will have height $\mathcal{O}(\log_B N)$. Note that the B-tree is a special case of the $(a,b)$-trees where the number of elements in leafs is a uniquely defined parameter.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.8\textwidth]{../figures/b_tree_fuse}
	\caption{Fusing a degree $a-1$ node $v$ (or leaf with $a-1$ elements) with sibling $v'$.}
	\label{fig:b_tree_fuse}
\end{figure}

A B-tree on $N$ elements are stored in $\mathcal{O}(\nicefrac{N}{B})$ blocks and can be constructed in the sorting bound by building the level level-by-level bottom-up.
...
\begin{figure}[h]
	...
	\caption{(a) First the $\leq M$ points are loaded into the main memory in $\mathcal{O}(M/B)$ and sorted (b) Then the two lists of sorted points are merged in $\mathcal{O}(M/B)$.}
...
After we have emptied all leaf-node buffers we remove the placeholder-elements one-by-one. The leaf-emptying invariant ensures that a node $v$ on the path from $u$ to the root has an empty buffer, but $v's$ sibling may not have an empty buffer. Therefore we can not fuse in a normal B-tree manner. Instead, we perform a buffer-emptying process on $v's$ immediate sibling before performing the actual fuse. See Figure~\ref{fig:buffer_tree_fuse}.
...
The results of MySQL and the external memory buffered priority search tree by Brodal align very well with the theoretical bounds, but this is clearly not the case for the structure of Arge et al. We suspect the bad performance of Arge et al is mainly due to the following reason. Every insert requires that we load every node on a path from the root to a leaf. Every node stores $B$ points and the tree has height $\mathcal{O}(\log_B N)$. It follows that every insert must load $\mathcal{O}(B\cdot \log_B N)$ points. For a buffer size of 4Kb and with 1Mb inserted we must load and flush $4Kb \cdot \log_{4Kb} 1Mb \approx 7Kb$. To insert 1Mb after 1Mb has already been inserted we must therefore approximately load and flush $7Kb \cdot 1Mb = 7Gb$. On top of this comes the cost of inserting into the query data structure which should be asymptotically similar but requires more internal work. If we compare this to the amount of data that has to be read and written in the case of the structure by Brodal with a similar buffer size we get that we need to load and store:
$$B \frac{1}{\epsilon} \log_B N = 4Kb \cdot \frac{1}{0.0833} \log_{4Kb} 1Mb \approx 
...
\begin{figure}[htp!]
\includegraphics[width=\textwidth]{../src/experiments/query_experiment_results/2016-05-05.21_00_12/time}
\caption{Theoretical query time for all tested structures.}
