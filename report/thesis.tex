...
\subsection{Stream}
The implementation we present makes use of the concept of \textit{streams}. Although the C++ standard library provides several streams that allow for the internal buffer to be managed we introduce an implementation of our own. We denote this stream \texttt{buffered\_stream}. This design choice was made because of the nature of the experiments we conduct in which it is of extreme importance that we are able to argue the running time in terms of the number of I/O's. By introducing a stream of our own we avoid that any undefined behaviour in the standard library implementations could give rise to an I/O overhead. Such an overhead would be reflected directly in the overall running time of our implementation.
...
In earlier experiments we have showed that a stream conceptually equal to \texttt{buffered\_stream} outperforms the basic streams provided in the standard library.
More precisely we conducted experiments on an I/O-optimal $\nicefrac{M}{B}$-way \textsc{Merge-Sorter} presented in~\cite{Aggarwal:1988/ICS/48529.48535} where we compared \texttt{B-stream} against the unbuffered \texttt{read} and \texttt{write} system calls, the standard library streams \texttt{fread} and \texttt{fwrite} with a built-in buffering mechanism and \texttt{mmap}/\texttt{munmap} that makes use of the operating systems virtual memory mechanism through demand paging. We believe the I/O pattern of linear scans through entire streams used in the $\nicefrac{M}{B}$-way \textsc{Merge-Sorter} matches our algorithmic set up and so we feel confident that the stream chosen performs well.
