...
Figure~\ref{fig:actual_insert_time} shows the actual running time per inserted megabyte in the all of the tested structures. The figure was cropped at $N = 600$ to better present the relation between the internal memory and the external memory structures. MySQL without an index was able to insert more than 10Gb 


 10Gb with no change in running time per megabyte as suspected. The same goes for the External Memory Buffered Priority Search by Brodal but it stopped at around 3500Mb when it had run for 24 hours.
