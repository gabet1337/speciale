...
This experiment focuses on the \textbf{report} of the query algorithm. We limited the experiment to only include Brodal's since we had to construct the data structure on a large input size. The experiment reused the idea of distributing data inside fixed query windows. 

\begin{figure}[h]
	\centering
	\includegraphics[width=0.8\textwidth]{../figures/query_uniform_fix_x_no_noise}
	\caption{MISSING !!!!!!!!!!!!!!! Distribution of points for the uniform reporting experiment (reporting). The points $x_1, x_2$ and $y_1, \cdots y_n$ are fixed. There is 1 Mb data in all ranges $[x_1, x_2] \times [y_i, y_{i+1}]$}.
	\label{fig:missing}
\end{figure}
