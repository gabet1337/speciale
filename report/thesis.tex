...
After all points have been reported we might have some buffers that are temporarily overflowed. This is now handled in a bottom up fashion using the update operations described in Subsection~\ref{subsec:gerth_updates}. We will handle a single subtree at a time and make sure that the entire subtree has no broken invariants, i.e.~buffers that overflow or underflow or any nodes with too high node degree.
 
\begin{figure}[h]
\centering
\includegraphics[width=0.80\textwidth]{../figures/gerth_fixup}
...
We do this by applying the update operations~\ref{update:del}-\ref{update:deg}\footnote{\begin{inlinelist}
	\item overflowing deletion buffers
	\item overflowing insertion buffers
	\item split leafs with overflowing point buffers
	\item split nodes of degree $\Delta+1$
\end{inlinelist}
...
\subsubsection*{Analysis}
During a query assume we visit $V$ nodes \textit{not on} the search paths for $x_1$ or $x_2$ and $\mathcal{O}\left(\frac{1}{\epsilon}\log_B N\right)$ nodes \textit{on} the search paths. We know that the $V$ nodes must have at least $VB/2$ points in their point buffers before updates are pushed down. The number of deletions we push down to visited nodes can at most be $\left(V + \mathcal{O}\left(\frac{1}{\epsilon}\log_B N\right)\right)B/4$.
...
$$VB/2 - \left(V + \mathcal{O}\left(\frac{1}{\epsilon}\log_B N\right)\right)B/4 = VB/4 - \mathcal{O}\left(\frac{B}{\epsilon}\log_B N\right) = K$$

By isolating $V$ it follows that $V = \mathcal{O}\left(\frac{1}{\epsilon}\log_B N + K/B\right)$.
...
We optimized the code as much as time allowed using the profiling tool \texttt{valgrind}.
...
\caption{Experimentally measured update time per operation for buffer sizes \\ $B \in \{1 \text{Mb}, 2 \text{Mb}, 4 \text{Mb}, 8 \text{Mb}, 16 \text{Mb}, 32 \text{Mb} \}$ with fanout 2. The tendencies align with the theoretical update bounds depicted in Figure~\ref{fig:gerth_buffer_size_theory}}
...
\caption{Experimentally measured I/O's per insert for buffer sizes \\ $B \in \{1 \text{Mb}, 2 \text{Mb}, 4 \text{Mb}, 8 \text{Mb}, 16 \text{Mb}, 32 \text{Mb} \}$ with fanout 2. The tendencies align with the theoretical update bounds depicted in Figure~\ref{fig:gerth_buffer_size_theory} and the actual update times depicted in Figure~\ref{fig:gerth_buffer_size_experiment}.}
...
\caption{Experimentally measured number of I/O's per insert as a function of input size with a fixed buffer size of 8Mb and varying fanouts $B^\epsilon~\in~\{2,3,4,5,6,8,16,32,64\}$. The tendencies align with the theoretical update bounds depicted in Figure~\ref{fig:gerth_fanout_experiment}.}
...
\begin{minipage}{.5\textwidth}
  \centering
  \includegraphics[]{../src/experiments/query_experiment_report_50mb_results/Final_100mb/time}
  \caption{}
...
\begin{minipage}{.5\textwidth}
  \centering
  \includegraphics[]{../src/experiments/query_experiment_report_50mb_results/2016-05-21.20_07_54/time}
...
\end{minipage}
\caption*{Reporting using the strategy of Figure~\ref{fig:experiment_query_uniform_fix_x} on data structures of size 100Mb (Figure~\ref{fig:query_report_100mb}). Here the Internal Priority Search Tree of McCreight suffers from swapping of memory. Running on data structures of size 400Mb (Figure~\ref{fig:query_report_400mb}). Here the Boost R-Tree suffers from swapping of memory.}
\end{figure}
