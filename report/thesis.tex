...
If the actual running time align with the theoretical bounds then we expect to see that the graphs forming horizontal lines. The caveat is that the internal memory data structures will start to swap out data when they have no more free main memory to use, which again causes the running time to increase severely. If we see a spike in both running time and number of page faults at the same mark then we feel confident that the to measures are closely related.
