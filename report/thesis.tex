...
The fundamental bounds in the I/O Model are that we can scan $N$ records in $\mathcal{O}(\text{Scan}) = \mathcal{O}(\nicefrac{N}{B})$ I/O's, sort $N$ records in $\mathcal{O}(\text{Sort}) = \mathcal{O}\left(\frac{N}{B} \log_{\nicefrac{M}{B}}\frac{N}{B}\right)$ I/O's, and search for a single record between $N$ records in $\mathcal{O}(\log_B N)$ I/O's. We denote $\mathcal{O}(\nicefrac{N}{B})$ as linear in terms of I/O's. Note that the $B$ factor is very important as $\nicefrac{N}{B} < \mathcal{O}\left(\frac{N}{B} \log_{\nicefrac{M}{B}}\frac{N}{B}\right) \ll N$.
