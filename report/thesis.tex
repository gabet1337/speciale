...
In the experiment conducted on the buffer size we fixed the fanout to 2 and varied the buffer size. Theoretically the running time should decrease with ever increasing buffer sizes as shown in Figure~\ref{fig:gerth_buffer_size_theory}. We are however limited by the main memory size and thus have to be careful not to cause swapping out memory to disk as this greatly decreases performance. 

The result plotted as running time per insert is depicted in Figure~\ref{fig:gerth_buffer_size_experiment}. Clearly the actual running time follows the same tendency as the theoretical number of I/O's per insert. We will focus on the number of I/O's to support this claim. In Figure ???


This statement is further backed by Figure~\ref{fig:gerth_buffer_size_experiment_ios_per_insert} where we depict the number of I/O's per insert.
...
\caption{Buffer size experiment. I/O's per insert.}
