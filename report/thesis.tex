...
Kasper, Gerth, A(man)duh
whoever might have suffered in the process. maybe move this section to after preface such that we can relate the suffering to the poor lady next door?
...
In this chapter we present important considerations noted throughout our implementation phase.

\section{General}
Almost all code was developed using pair programming and we strongly believe that this process, though slow and cumbersome, eliminates many mistakes that would otherwise slow us down later on.
Everything we implemented was unit tested and large parts of the project was implemented using test driven development. We made sure to make sound design choices to enable easy extension and reuse of our code. Stubs and mocks were used to ease the integration of substructures into larger pieces.
...
We also implemented wrappers around MySQL, Boost R-Tree, and libspatial external R*-tree such that we could use these structures as a priority search tree. These structures are described further in Chapter~\ref{chp:other_structures}.

\section{Test framework}
\todo{describe the test framework}
...
Many experiments were conducted to evaluate the performance of the structures against each other. In this chapter we present and discuss the most interesting results of these experiments. Some of the experiments have been time limited such that we only allow the test to run for fixed amount of time. This was a necessary restriction as the internal memory structures are severely limited when data input is greater than the available main memory. This will be very apparent in many of the presented results.

\section{Parameter tuning}
Both the original and buffered external memory priority search tree by Arge et al.~(Chapter~\ref{chp:arge_pst}) and Brodal (Chapter~\ref{chp:epst}) respectively, are parametrised with fanout and buffer size. These parameters are highly machine dependent as larger main memory allows for larger buffer sizes. Therefore we conducted an experiment for finding the optimal parameters on our test farm.
...
We decided to focus the parameter tuning sorely on the \textsc{insert} operation, since this would allow us to construct data structures with a large amount of data, which again would give rise to interesting experiments for the remaining operations. Put in other words; it is of no interest to achieve a data structure that queries \textit{really} fast if we are unable to construct it with a decent amount of data within a decent amount of time.
...
In the experiment conducted on the buffer size we fixed the fanout to 2 and varied the buffer size. Theoretically the running time should decrease with ever increasing buffer sizes as shown in Figure~\ref{fig:gerth_buffer_size_theory}. We are however limited by the main memory size and thus have to be careful not to cause swapping out memory to disk as this greatly decreases performance. This is the reason behind the parameter being of a very machine dependent nature; we want to utilize as much main memory as available in the machine running the data structure.

The result plotted as running time per insert is depicted in Figure~\ref{fig:gerth_buffer_size_experiment}. Clearly the actual running time aligns very well with the theoretical running time. This statement is further backed by Figure ??? where we have divided the actual running and expected times, resulting in almost straight lines for all buffer sizes.

We see the effect of the limited amount of internal memory when we reach 16Mb. We would have expected a significant increase in performance compared to 8Mb but instead we see that 8Mb performs just as well as 16Mb. EXPLAIN WITH SWAPPING AND PAGE FAULTS AND ALL THAT JAZZ.
...
\begin{figure}[htp]
\centering
\includegraphics[width=0.95\textwidth]{../plots/gerth_update_buffer_size/gerth_buffer_size}
