...
Please refer to figure ??? for an illustration of the categories. Querying is done by locating the nodes intersecting the left side of the query using the B-tree. Points from the nodes are reported by examining the associated buckets. Next, right siblings of the nodes and their descendants are examined in a top-down fashion until the bottom boundary of the query is crossed. It is crucial to note that the corner, ancestor, and sibling nodes can cause wasteful I/O's. Thus there are $\mathcal{O}(\log n)$ wasteful nodes as the every parent of a visited node would have contributed an useful I/O. From this analysis we can conclude that we can answer 2-sided queries on $\mathcal{O}(\log n + K/B)$ I/O's. Now, they are able to avoid the wasteful I/O's by caching the data in the ancestor and sibling nodes. By associating two caches are with the corner that contain all data in the siblings stored sorted by $x$-coordinate and $y$-coordinate respectively, we are able to answer queries in $\log_B n + K/B$ I/O's by simply locating the corner in $\mathcal{O}(\log_B n)$ I/O's and report using the cache. The storage use is $\mathcal{O}(\nicefrac{n}{B} \log n)$ disk blocks of size $B$ each. The idea can be extended to handle 3-sided queries by adding additional caches that cover point sets sorted on $y$-coordinate from right to left. This gives a space usage of $\mathcal{O}(\nicefrac{n}{B} \log^2 n)$. The space usage is brought down by building a search tree that divides the points into regions of size $B \log B$ instead of B giving a total of $\nicefrac{n}{B} \log B$ regions. Using a slightly modified caching scheme and by constructing a secondary level structure for each region the space can be brought down to $\mathcal{O}(\nicefrac{n}{B} \log \log B)$. Reusing this idea using a multilevel scheme brings down the data structure to a $\mathcal{O}(\nicefrac{n}{B} \log B \log^* B)$ space solution that answers queries in $\mathcal{O}(\log_B + K/B + \log^* B)$.
