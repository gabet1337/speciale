...
%What Equation~\ref{eq:equal_fanouts_2_4_1} and Equation~\ref{eq:equal_fanouts_2_4_2} essential states is that we expect no performance gain when recursively sending half as many points from root to leaf in a tree of double the height.\todo{reformulate this sentence. It is not correct.}
...
\begin{minipage}[t]{0.48\columnwidth}
  ...
  \caption{Theoretical asymptotic update time per operation for buffer sizes $B~\in~\{4\text{Kb},8\text{Kb},16\text{Kb},32\text{Kb}\}$ for Arge's structure. Each graph is on the form $f(N) = \log_B N$.\todo{FIX THIS}}
