...
In the experiment conducted on the fanout parameter we fixed the buffer size to 1 Mb and varied the fanout parameter. Now, ones intuition might deceive one into believing that an ever increasing fanout would cause an ever decreasing update time per operation. This was certainly what we had expected since we pass down an ever decreasing $B / B^\epsilon$ fraction, and each such fraction charges an I/O to the total running time. In Figure~\ref{fig:gerth_fanout_experiment} we present the theoretical update time per operation. We see a clear tendency of ever increasing fanouts causing an ever decreasing running time for $B^\epsilon \in \{4, 5, 8, 16, 32, 64 \}$. When zooming in on the graphs for $B^\epsilon \in \{2, 3, 4, 5\}$ we see that no theoretical gain is achieved when decreasing the fanout from 3 to 2. In fact it seems that we achieve the exact same update time per operation for fanout 2 and fanout 4. This is, however, not as surprising as one might think. See Equation~\ref{eq:equal_fanouts_2_4} where it is shown that the theoretical bound is the exact same for fanouts 2 and 4.

\begin{equation} \label{eq:equal_fanouts_2_4}
	\begin{split}
		\frac{1}{\epsilon \cdot B^{1-\epsilon}}\log_B N & = \frac{B^\epsilon}{B}\frac{1}{\epsilon}\log_B N \xRightarrow{B^\epsilon = 2\text{\ \ }} \\
		\frac{1}{\epsilon \cdot B^{1-\epsilon}}\log_B N & = \frac{2\log_2(B)}{B\log_2(2)}\log_B N \\
		& = \frac{4\log_2(B)}{2B\log_2(2)}\log_B N \\
		& = \frac{4\log_2(B)}{B\log_2(2^2)}\log_B N \\
		& = \frac{4\log_2(B)}{B\log_2(4)}\log_B N \\
		& \Rightarrow B^\epsilon = 2 = 4
	\end{split}
\end{equation}

What Equation~\ref{eq:equal_fanouts_2_4} states is essential that we expect no performance gain when recursively sending half as many points from root to leaf in a tree of double the height. \todo{NOT SURE!}.
