...
\label{chp:epst}
In this section we present an external memory data structure described by Brodal in~\cite{DBLP:journals/corr/Brodal15}. This structure supports updates in amortized $\mathcal{O}\left(\frac{1}{\epsilon B^{1-\epsilon}} \log_B N\right)$, three sided range queries in $\mathcal{O}\left(\frac{1}{\epsilon}\log_B N + \nicefrac{K}{B}\right)$ for $0 \leq \epsilon \leq \frac{1}{2}$, and can be constructed on $N$ points in the sorting bound. $\epsilon$ is a parameter that determines the size of the fanout and in turn the size of a bootstrapped substructure for storing $\mathcal{O}(B^{1+\epsilon})$ points, $0 \leq \epsilon \leq \frac{1}{2}$.
The substructure is very similar to that of Arge et al.~\cite[Section~3.1]{arge_vitter_2003} for handling $\mathcal{O}(B^2)$ points with the main difference being that we reduce the capacity to allow an amortized constant number of I/O's per update. The bootstrapped structure is in ~\cite{arge_vitter_2003} used to store the top $\mathcal{O}(B^2)$ points w.r.t. $y$ value for the subtree rooted at the given node of the substructure. This structure uses it in a slightly different way to store the top $\mathcal{O}(B^{1+\epsilon})$ points of the children of the given  node. This structure is described further in Section~\ref{sec:child_structure}.
...
There exists a dynamic data structure for storing $\mathcal{O}(B^{1+\epsilon})$ two dimensional points, $0 \leq \epsilon \leq \frac{1}{2}$.
...
\todo{Kasper: Troede det var $B^{1+\epsilon}$ points??}
...
The subtree at node $v$ contains points in range spanned by the points in the point buffer $P_v$, i.e. the structure is a search tree on the point in $P_v$ w.r.t. to the normal lexicographical ordering of points.
...
\begin{itemize}
	...
	\todo{Kasper: Hvad betyder det? Maaske skal i beskrive mere grundigt ovenfor at det er et soegetrae paa x. PETER: Er gjort ovenfor. Godt nok?}.
...
Once the set of points to reinsert have been found be simply insert the points on an initially empty tree.

\textbf{Analysis}

\todo{Kasper:  Er der altid lige langt til alle leaves?}
...
\todo{Kasper: Ikke helt klart hvad der menes her}
\textbf{Analysis}
