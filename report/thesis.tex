...
\subsection{Stream}
The implementation we present makes use of the concept of \textit{streams}. Although the C++ standard library provides several streams that allow for the internal buffer to be managed we introduce an implementation of our own. We denote this stream \texttt{B-stream}. This design choice was made because of the nature of the experiments we conduct in which it is of extreme importance that we are able to argue the running time in terms of the number of I/O's. By introducing a stream of our own we avoid that any undefined behaviour in the standard library implementations could give raise to an I/O overhead. Such an overhead would be reflected directly in the overall running time of our implementation.
